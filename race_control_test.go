...
func TestRaceControl_OnClientConnect(t *testing.T) {
	t.Run("Client first connect", func(t *testing.T) {
		...
		raceControl := NewRaceControl(NilBroadcaster{}, nilTrackData{}, dummyServerProcess{}, testStore, NewPenaltiesManager(testStore), &dummyNotificationManager{})
...
func TestRaceControl_SortDrivers(t *testing.T) {
	t.Run("Race, connected drivers", func(t *testing.T) {
		...
		srm := NewScheduledRacesManager(testStore)
		dm, _ := NewDiscordManager(testStore, srm)
		nm := NewNotificationManager(dm, cm, testStore)

		rc := NewRaceControl(NilBroadcaster{}, nilTrackData{}, dummyServerProcess{}, testStore, NewPenaltiesManager(testStore), nm)
	...
	t.Run("Non-race, connected drivers", func(t *testing.T) {
		t.Run("Two drivers with valid laps, two without", func(t *testing.T) {
			rc := NewRaceControl(NilBroadcaster{}, nilTrackData{}, dummyServerProcess{}, testStore, NewPenaltiesManager(testStore), &dummyNotificationManager{})
